...
def main():
    ...
    ax1.set_title(f'Hessian Determinant Zero Set\n{len(boxes)} boxes on [0,1]²', fontsize=14)
    ax1.set_xlim(0, 1)
    ax1.set_ylim(0, 1)
    ...
    # Draw boxes in original coordinates
    for box in boxes:
        u_min, u_max, v_min, v_max = box
        x_min = 2 * u_min - 1
        x_max = 2 * u_max - 1
    ...
    ax2.set_title(f'Hessian Determinant Zero Set\n{len(boxes)} boxes on [-1,1]²', fontsize=14)
    ax2.set_xlim(-1, 1)
    ax2.set_ylim(-1, 1)
